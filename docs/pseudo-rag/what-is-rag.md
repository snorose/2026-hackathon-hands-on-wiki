...
이번 실습은 이 중에서 “참고자료를 AI에게 전달한다”는 핵심만 다룹니다. 문서 저장소를 만들거나 관련 문서를 자동 검색하지는 않습니다.

그래서 이 문서에서는 이 방식을 **Pseudo-RAG(간이 RAG)** 라고 부릅니다. 표준 용어라기보다는, 실제 RAG의 전체 구조를 만들기 전에 “참고자료를 근거로 답하게 한다”는 감각을 이해하기 위한 설명용 표현입니다.

```mermaid
flowchart TB
    subgraph lite["이번 실습: Pseudo-RAG"]
